...
class OpenCVCamera(Camera):
    ...
    def _configure_capture_settings(self) -> None:
        ...
        self._apply_optional_camera_controls()

    def _apply_optional_camera_controls(self) -> None:
        if self.auto_exposure is not None:
            success = self.videocapture.set(cv2.CAP_PROP_AUTO_EXPOSURE, float(self.auto_exposure))
            actual = self.videocapture.get(cv2.CAP_PROP_AUTO_EXPOSURE)
            logger.info(f"{self} set auto_exposure={self.auto_exposure} ({actual=}, {success=}).")

        if self.exposure is not None:
            success = self.videocapture.set(cv2.CAP_PROP_EXPOSURE, float(self.exposure))
            actual = self.videocapture.get(cv2.CAP_PROP_EXPOSURE)
            logger.info(f"{self} set exposure={self.exposure} ({actual=}, {success=}).")

        if self.gain is not None:
            success = self.videocapture.set(cv2.CAP_PROP_GAIN, float(self.gain))
            actual = self.videocapture.get(cv2.CAP_PROP_GAIN)
            logger.info(f"{self} set gain={self.gain} ({actual=}, {success=}).")
